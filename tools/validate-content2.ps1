...
Get-ChildItem $postsDir -Filter *.md | ForEach-Object {
  ...
  # Allow date-prefixed filenames like 2025-09-08-my-post.md
  $fileSlugSansDate = $fileSlug
  $m = [regex]::Match($fileSlug, '^\d{4}-\d{2}-\d{2}-(.+)$')
  if($m.Success){ $fileSlugSansDate = $m.Groups[1].Value }

  $slug = [string](FM $meta 'slug')
  if([string]::IsNullOrWhiteSpace($slug)){ $slug = $fileSlugSansDate }

  if(($slug -ne $fileSlug) -and ($slug -ne $fileSlugSansDate)){
    $errors += "[$($md.Name)] slug '$slug' does not match filename '$fileSlug' (or '$fileSlugSansDate' after stripping date)."
  }
  ...
  # cover (warn only) -> must be in static/images/hero if provided
  ...
  foreach($m2 in $matches){
    $fname = $m2.Groups[1].Value
